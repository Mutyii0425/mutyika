...
import { useNavigate } from 'react-router-dom';
  const Home = () => {
    ...
    const prizes = [
      ...
      { option: '10% kedvezmény', style: { backgroundColor: '#2ECC71', textColor: '#fff' }},
      { option: '5% kedvezmény', style: { backgroundColor: '#34495E', textColor: '#fff' }},
      { option: 'Nincs nyeremény', style: { backgroundColor: '#2ECC71', textColor: '#fff' }},
      { option: '20% kedvezmény', style: { backgroundColor: '#34495E', textColor: '#fff' }},
      { option: '15% kedvezmény', style: { backgroundColor: '#2ECC71', textColor: '#fff' }}
    ];
  ...
  const spinCoupon = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (spins >= maxSpins) {
        ...
        // Frissítjük a localStorage-t és minden state-et egyszerre
        const user = JSON.parse(localStorage.getItem('user'));
        user.hasWonPrize = true;
        user.kupon = finalPrize;
        delete user.isNewRegistration;
        ...
        setTimeout(() => {
          setIsSpinning(false);
          // Wait additional 1.5 seconds before showing success alert
          setTimeout(() => {
            saveCouponToDatabase(finalPrize);
            setShowSuccessAlert(true);
            onClose();
    ...
    useEffect(() => {
      ...
      if (userData) {
        const user = JSON.parse(userData);
        // Csak akkor mutatjuk a pörgetőt, ha új regisztráció és még nem pörgetett
        if (user.isNewRegistration && !spinComplete) {
          setShowWelcomeDialog(true);
        }
      }
    }, [spinComplete]);
